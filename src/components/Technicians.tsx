...
export default function Technicians({
    ...
    return [
      {
        ...
        reportedIssue:
          "High density cockroach infestation and drain flies in catering base",
  ...
  const [activeTabSheet, setActiveTabSheet] = useState<"plans" | "completed">(
    "plans",
  );
  ...
  useEffect(() => {
    ...
    if (serverLoadedRef.current.plans) {
      saveStoreValue("field_plans", plans).catch((err) =>
        console.log("Failed to sync plans to Firestore:", err),
      );
    }
  }, [plans]);
  ...
  useEffect(() => {
    ...
    if (serverLoadedRef.current.feed) {
      saveStoreValue("timeline_feed", timelineFeed).catch((err) =>
        console.log("Failed to sync feed to Firestore:", err),
      );
    }
  }, [timelineFeed]);
...
- Perform safe exterior drainage pipeline cleaning and insect liquid control.`;
  ...
  const [dutyContacts, setDutyContacts] = useState<
  ...
  const handleFacilitySelect = (val: string) => {
    ...
    if (val) {
      ...
      setReportedIssue(
        isBengali ? "কাজের বিবরণ ও নির্দেশনা" : "Tasks Description",
      );
    }
  };
  ...
  const handleCreateNewFieldPlan = (e: React.FormEvent) => {
    ...
    if (!targetFacility) {
      triggerToast(
        isBengali
          ? "অনুগ্রহ করে একটি সেন্টার বা হসপিটাল সিলেক্ট করুন!"
          : "Please select or type a target facility center!",
      );
      return;
    }
    ...
    const finalInstructions =
      ...
      (isBengali ? "সম্পূর্ণ সাধারণ পেস্ট কন্ট্রোল" : "General Pest Control");
    ...
    triggerToast(
      isBengali
        ? `✓ ${targetFacility} এর জন্য নতুন প্ল্যান সফলভাবে এক্সেল আকারে সংযুক্ত হয়েছে!`
        : `✓ Plan for ${targetFacility} loaded to active spreadsheet!`,
    );
  ...
  const handleGoToFormForPlan = (plan: FieldPlanItem) => {
    if (onSelectClientToPrefill) {
      ...
      triggerToast(
        isBengali
          ? `✓ ${plan.facilityName} এর জন্য ডেইলি রিপোর্ট ফর্মে রিডাইরেক্ট করা হচ্ছে...`
          : `✓ Redirecting to Daily Service Sheet for ${plan.facilityName}...`,
      );
    } else {
  ...
  const handleDeletePlan = (planId: string, facilityName: string) => {
    ...
    triggerToast(
      isBengali
        ? `✓ ${facilityName} এর প্ল্যানটি মুছে ফেলা হয়েছে।`
        : `✓ Plan for ${facilityName} deleted.`,
    );
  };
  ...
  const handleDeleteTimelineEntry = (feedId: string) => {
    ...
    triggerToast(
      isBengali
        ? "✓ টাইমলাইন ইনফরমেশন মুছে ফেলা হয়েছে।"
        : "✓ Timeline entry cleared.",
    );
  };
  ...
  const quickChooseTemplates = [
    {
      ...
      descBengali:
        "এখনও কাজ পুরোপুরি সমাধান শেষ হয়নি। কিছু কেমিক্যালের জটিলতা ও অতিরিক্ত স্যানিটেশন বাকি আছে।",
      descEnglish:
        "Work is not yet fully finished. Some chemical issues and sanitization remain.",
    },
    {
      id: "T2",
      label: "২. আংশিক কাজ হয়েছে, কিছু জায়গায় কাজ বাকি রয়েছে।",
      descBengali:
        "আংশিক কাজ সম্পন্ন হয়েছে, কিছু স্যানিটারি ড্রেন স্লিভ পাইপ ও বাথরুম প্রুফিং বাকি রয়েছে।",
      descEnglish:
        "Partial work completed, some sanitary drain tubes and toilet proofing remain.",
    },
    {
      id: "T3",
      label: "৩. কেমিক্যালের সমস্যা ছিল",
      descBengali:
        "সেন্টারে কেমিক্যালের সমস্যা ছিল, যার দরুন বাকি কেমিক্যাল স্প্রে পরবর্তী শিফটে সম্পন্ন করতে হবে।",
      descEnglish:
        "There was a chemical asset issue, remaining spray postponed to next shift.",
    },
    {
      id: "T4",
      label: "৪. অতিরিক্ত ময়লা থাকার কারণে কাজটি পুরোপুরি করা সম্ভব হয়নি।",
      descBengali:
        "অতিরিক্ত ময়লা থাকার কারণে কাজটি পুরোপুরি করা সম্ভব হয়নি। রোগজীবাণু মুক্ত করতে পরিষ্কার কাজের নির্দেশনা দেয়া হয়েছে।",
      descEnglish:
        "Extreme debris/dirt prevented complete treatment. Instructed deep cleaning.",
    },
  ];
  ...
  const handleSelectQuickTemplate = (
    tmpl: (typeof quickChooseTemplates)[0],
  ) => {
  ...
  const handleSubmitProgress = (isPartial: boolean) => {
    ...
    if (onUpdateReports) {
      ...
      const newReportLog: ReportItem = {
        ...
        emirate: updatingPlan.facilityName.toLowerCase().includes("dubai")
          ? "Dubai"
          : updatingPlan.facilityName.toLowerCase().includes("sharjah")
            ? "Sharjah"
            : "Ajman",
        ...
        recommendations: [
          isPartial
            ? "Complete remaining slot proofer lines in upcoming shifts."
            : "Audit safe sanitization regularly.",
        ],
        billing: {
  ...
  const [activeFormPrintItem, setActiveFormPrintItem] = useState<any | null>(
    null,
  );
  ...
  const planCenters = plans
    .map((p) => String((p as any).centerName || ""))
    .filter((name) => !!name);
  ...
  useEffect(() => {
    if (activeWhatsAppTab === "staff") {
      if (selectedWhatsAppContact) {
        const contact = dutyContacts.find(
          (c) => c.id === selectedWhatsAppContact,
        );
        if (contact) {
          let msg = isBengali
          ...
          const todayPlans = plans.filter(
            (p) => p.shift?.toLowerCase() === "today",
          );
          const tomorrowPlans = plans.filter(
            (p) => p.shift?.toLowerCase() === "tomorrow",
          );
          const otherPlans = plans.filter(
            (p) =>
              p.shift?.toLowerCase() !== "today" &&
              p.shift?.toLowerCase() !== "tomorrow",
          );
  ...
  const filteredPlans = plans.filter(
    (p) =>
      ...
      (p.workAreas &&
        p.workAreas.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (p.reportedIssue &&
        p.reportedIssue.toLowerCase().includes(searchQuery.toLowerCase())) ||
      p.assignedTo.toLowerCase().includes(searchQuery.toLowerCase()),
  );

  const filteredFeed = timelineFeed.filter(
    (f) =>
      f.facilityName.toLowerCase().includes(searchQuery.toLowerCase()) ||
      f.postedBy.toLowerCase().includes(searchQuery.toLowerCase()) ||
      (f.workAreas &&
        f.workAreas.toLowerCase().includes(searchQuery.toLowerCase())) ||
      f.doneText.toLowerCase().includes(searchQuery.toLowerCase()),
  );

  return (
    <div
      id="erp-planning-timeline-dashboard"
      className="space-y-6 pb-16 font-sans antialiased text-slate-900"
    >
      {/* Global Toast Notification */}
      {toastMessage && (
        <div
          id="timeline-global-toast"
          className="fixed top-6 right-6 z-[160] bg-slate-900 border border-slate-700 text-white p-4 rounded-xl shadow-2xl flex items-center gap-3 animate-slide-left max-w-sm"
        >
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="text-[9px] bg-slate-100 text-slate-850 font-mono font-black tracking-widest uppercase px-2.5 py-1 rounded-sm border border-slate-350 flex items-center gap-1">
              ...
              {isBengali
                ? "রিয়েল-টাইম কাজের খতিয়ান ও এক্সেল ট্র্যাকার"
                : "LIVE FIELD WORKBOOK"}
            </span>
          </div>
          <h1 className="text-2xl font-black tracking-tight text-slate-950 flex items-center gap-2 mt-1">
            ...
            <span>
              {isBengali
                ? "পরিকল্পনা ও টাইমলাইন এটেনশন"
                : "Planning & Timeline Attention"}
            </span>
          </h1>
          ...
          <div className="bg-white border-2 border-slate-900 p-3 rounded-lg flex flex-col sm:flex-row justify-between items-center gap-3">
            <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto relative">
              ...
              >
                <span>
                  ➕{" "}
                  {isBengali
                    ? "নতুন কাজের শিডিউল তৈরি করুন"
                    : "Schedule New Route Plan"}
                </span>
              </button>
                  ...
                  <div className="absolute top-full left-0 mt-2 w-[300px] bg-white border-2 border-slate-900 rounded-xl shadow-2xl z-50 overflow-hidden font-sans">
                    ...
                    <div className="max-h-[300px] overflow-y-auto p-2 space-y-2 bg-slate-50">
                      {dutyContacts.map((c) => (
                        <div
                          key={c.id}
                          className="flex flex-col bg-white border border-slate-200 rounded p-2 shadow-xs group"
                        >
                          <div className="flex justify-between items-start">
                            ...
                            <button
                              ...
                              className="text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity p-0.5 shrink-0"
                            >
                          ...
                          <div className="mt-1.5 flex items-center justify-between">
                            <a
                              href={`tel:${c.phone}`}
                              className="flex items-center gap-1.5 text-[11px] text-[#2563EB] hover:text-blue-700 font-bold bg-blue-50 px-2 py-1 rounded-md transition-colors border border-blue-100"
                            >
                      ...
                      {dutyContacts.length === 0 && (
                        <div className="text-center text-slate-400 text-xs py-4 italic font-medium">
                          {isBengali ? "কোনো নাম্বার নেই" : "No contacts added"}
                        </div>
                      )}
                    </div>
                    <div className="p-3 bg-slate-100 border-t border-slate-200 space-y-2 flex flex-col gap-2">
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="w-full text-[11px] font-medium border border-slate-300 px-2.5 py-1.5 rounded-md outline-none focus:border-blue-500 placeholder-slate-400"
                        />
                        <select
                          value={newContactRole}
                          onChange={(e) => setNewContactRole(e.target.value)}
                          className="text-[11px] font-medium border border-slate-300 px-2 py-1.5 rounded-md outline-none focus:border-blue-500 cursor-pointer bg-white text-slate-700"
                        >
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="bg-emerald-600 hover:bg-emerald-500 text-white px-3 py-1.5 rounded-md font-black text-[11px] flex items-center justify-center gap-1 shrink-0 transition-colors cursor-pointer shadow-sm"
                        >
            ...
            <div className="relative w-full sm:w-64">
              <input
                ...
                placeholder={
                  isBengali
                    ? "হাসপাতাল বা টেকনিশিয়ান নাম খুঁজুন..."
            ...
            <div className="p-3 bg-white overflow-x-auto min-h-[480px]">
                  ...
                  <table className="w-full text-left border-collapse border border-slate-300 font-mono text-[10.5px] select-text">
                    <thead>
                      <tr className="bg-slate-50/80 text-slate-700">
                        <th className="border border-slate-300 p-1 bg-slate-100 font-extrabold text-center w-8">
                          A
                        </th>
                        <th className="border border-slate-300 p-1.5 font-black uppercase text-slate-950">
                          Plan ID / Center Name
                        </th>
                        <th className="border border-slate-300 p-1.5 font-black uppercase text-slate-950">
                          Shift
                        ...
                        <th className="border border-slate-300 p-1.5 font-black uppercase text-slate-950">
                          {isBengali ? "কাজের স্থান" : "Work Areas"}
                        </th>
                        <th className="border border-slate-300 p-1.5 font-black uppercase text-slate-950">
                          Reported Problem
                        </th>
                        <th className="border border-slate-300 p-1.5 font-black uppercase text-slate-950 text-center">
                          Action
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredPlans.length === 0 ? (
                        <tr>
                          <td
                            colSpan={6}
                            className="border border-slate-300 p-8 text-center text-slate-400 select-none italic font-sans text-xs"
                          >
                            {isBengali
                              ? "কোনো সক্রিয় রুট পরিকল্পনা মেলেনি!"
                              : "No scheduled excel entries found."}
                          </td>
                        </tr>
                      ) : (
                            ...
                            <React.Fragment key={plan.id}>
                              <tr
                                ...
                                className={`group border-b border-slate-300 transition-colors cursor-pointer text-slate-900 ${
                                  isCompleted
                                    ...
                                    : isSelected
                                      ? "bg-indigo-50/80 hover:bg-indigo-100/30"
                                      : "hover:bg-slate-50"
                                }`}
                              >
                                ...
                                <td className="border border-slate-300 p-2 max-w-xs font-bold">
                                  <div className="font-mono text-[9px] text-[#2563EB] font-black">
                                    {plan.id}
                                  </div>
                                  <div className="font-sans font-black text-slate-950 leading-snug">
                                    {plan.facilityName}
                                  </div>
                                  <div className="text-[9px] text-slate-400 font-sans mt-0.5 font-bold">
                                    Assigned:{" "}
                                ...
                                <td className="border border-slate-300 p-2 font-extrabold">
                                  <span
                                    className={`px-1.5 py-0.5 rounded text-[8.5px] font-sans font-black uppercase ${
                                      plan.dateFrame === "Today"
                                        ? "bg-emerald-600/10 text-emerald-800"
                                        : plan.dateFrame === "Tomorrow"
                                ...
                                <td
                                  className="border border-slate-300 p-2 font-semibold text-slate-705 leading-snug font-sans whitespace-pre-wrap break-words min-w-[120px]"
                                  title={plan.workAreas}
                                >
                                  {plan.workAreas || "-"}
                                </td>
                                ...
                                <td
                                  className="border border-slate-300 p-2 text-slate-900 font-bold leading-snug font-sans whitespace-pre-wrap break-words min-w-[180px]"
                                  title={
                                    plan.instructionText ||
                                    plan.notes ||
                                    plan.reportedIssue
                                  }
                                >
                                  ...
                                  <div className="flex justify-center items-center gap-1">
                                    ...
                                    {!isCompleted ? (
                                      <button
                                        ...
                                        title={
                                          isBengali
                                            ? "ডেইলি রিপোর্ট ফর্মে যান"
                                            : "Go to Daily Entry Form Sheet"
                                        }
                                        onClick={() =>
                                          handleGoToFormForPlan(plan)
                                        }
                              ...
                              {isSelected && (
                                <tr className="bg-slate-50/50">
                                  ...
                                  <td
                                    colSpan={5}
                                    className="border border-slate-300 p-3.5 text-left font-sans space-y-3"
                                  >
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                      <div className="space-y-1 bg-white p-2.5 rounded border">
                                        <b className="text-[10px] text-indigo-700 uppercase tracking-widest font-mono block">
                                          📍{" "}
                                          {isBengali
                                            ? "কোথায় কোথায় কাজ করতে হবে:"
                                            : "Work Areas To Treat:"}
                                        </b>
                                        <p className="text-xs font-black text-slate-900 leading-relaxed bg-slate-50 p-2 rounded">
                                          {plan.workAreas || "Not Specified"}
                                        </p>
                                      </div>

                                      <div className="space-y-1 bg-white p-2.5 rounded border">
                                        <b className="text-[10px] text-[#2563EB] uppercase tracking-widest font-mono block">
                                          ⚠️{" "}
                                          {isBengali
                                            ? "কী সমস্যা হয়েছে:"
                                            : "Reported Problem:"}
                                        </b>
                                        <p className="text-xs font-black text-slate-900 leading-relaxed bg-rose-50/20 p-2 rounded">
                                          {plan.instructionText ||
                                            plan.notes ||
                                            plan.reportedIssue ||
                                            "Not Specified"}
                                    ...
                                    <div className="space-y-2.5 bg-white p-3 rounded border">
                                      <div className="flex justify-between items-center border-b pb-1.5">
                                        <span className="text-[10px] text-slate-550 font-black font-mono">
                                          SPECIFIED ASSIGNED PRESCRIPTIONS &
                                          TASKS:
                                        </span>
                                        <span className="text-[9px] bg-indigo-100 text-indigo-850 font-mono font-bold px-1.5 rounded">
                                          {plan.tasks.length} Duties
                                        </span>
                                      </div>
                                      <div className="flex flex-wrap gap-1.5">
                                        {plan.tasks.map((t, idx) => (
                                          <span
                                            key={idx}
                                            className="text-xs font-black bg-slate-100 text-slate-800 border-2 border-slate-200 py-1 px-2.5 rounded-lg"
                                          >
                                            ✓ {t}
                                          </span>
                                        ))}
                ...
                <div className="space-y-4 animate-fadeIn">
                  <div className="flex justify-between items-center pb-2 border-b border-slate-200">
                    <span className="text-[10px] font-mono text-slate-500 block uppercase font-bold">
                      *{" "}
                      {isBengali
                        ? "সম্পন্ন কাজের এক্সেল বিবরণ"
                        : "Completed Services Timeline logs"}
                    </span>
                    <span className="text-[9.5px] text-[#2563EB] bg-blue-50 px-2.5 py-1 rounded-lg border-2 border-blue-200 font-black uppercase font-mono">
                      {isBengali ? "সংরক্ষিত সেন্টার:" : "Archived Logs:"}{" "}
                      {filteredFeed.length}
                    </span>
                  </div>
                  ...
                  {filteredFeed.length === 0 ? (
                    <div className="border border-dashed border-slate-200 p-12 text-center text-slate-400 select-none italic font-sans text-xs rounded-xl bg-slate-50">
                      {isBengali
                        ? "কোনো সম্পন্ন সার্ভিস খতিয়ান পাওয়া যায়নি!"
                        : "No archived completed timelines yet."}
                    </div>
                  ) : (
                    <div className="space-y-3 font-sans">
                      {filteredFeed.map((feed) => {
                        ...
                        const timing =
                          feed.timestamp.includes("Tomorrow") ||
                          feed.timestamp.toLowerCase().includes("tomorrow") ||
                          feed.dateFrame?.includes("Tomorrow")
                            ? "Tomorrow"
                            : "Today";
                        const isTom =
                          feed.dateFrame === "Tomorrow" ||
                          timing === "Tomorrow";
                        const timingBn = isTom
                          ? "আগামীকাল"
                          : feed.dateFrame === "Today"
                            ? "আজ"
                            : "পরশু";
                        const timingEn = feed.dateFrame || timing;

                        const loc =
                          feed.locationType || feed.workAreas || "Inside";
                        const locBn = loc.toLowerCase().includes("outside")
                          ? "আউটসাইড"
                          : loc.toLowerCase().includes("both")
                            ? "ইনসাইড ও আউটসাইড"
                            : "ইনসাইড";
                        const locEn = loc;
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2 w-full sm:w-auto justify-end border-t border-slate-800 pt-3 sm:pt-0 sm:border-0 shrink-0">
                              <button
                                ...
                                onClick={() =>
                                  handleOpenPrintForm(feed, "completed")
                                }
                              ...
                              >
                                <span>📄</span>
                                <span>
                                  {isBengali ? "ফরম দেখুন" : "View Voucher"}
                                </span>
                              </button>
                              <button
                                type="button"
                                ...
                                onClick={() =>
                                  handleDeleteTimelineEntry(feed.id)
                                }
              ...
              <div className="mt-4 p-3 bg-slate-50 border border-slate-300 rounded text-[10.5px] text-slate-500 leading-relaxed font-bold font-sans">
                ...
                <span className="text-slate-800">
                  {isBengali ? "এক্সেল নির্দেশক:" : "Worksheet Note:"}
                </span>{" "}
                {isBengali
              ...
              {activeWhatsAppTab === "staff" ? (
                <>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setSelectedWhatsAppContact(e.target.value)
                      }
                    >
                      <option value="" disabled>
                        {isBengali
                          ? "লিস্ট থেকে কন্টাক্ট সিলেক্ট করুন..."
                          : "Select a contact..."}
                      </option>
                      {dutyContacts.map((c) => (
                    ...
                    {dutyContacts.length === 0 && (
                      <p className="text-[10px] text-rose-500 font-bold">
                        {isBengali
                          ? "লিস্টে কোনো নাম্বার এড নেই। উপরের কন্টাক্ট আইকনে গিয়ে নাম্বার এড করুন।"
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">
                      {isBengali
                        ? "মেসেজ লিখুন (কাস্টমাইজ করতে পারবেন)"
                        : "Message Content"}
                    </label>
                    <textarea
                ...
                <div className="space-y-3">
                  <p className="text-[11.5px] text-slate-500 font-bold mb-2">
                    {isBengali
                      ? "আজ এবং আগামীকালের সিডিউল করা সেন্টারগুলোতে নোটিফিকেশন পাঠান।"
                      : "Send notification to currently scheduled centers."}
                  </p>
                  {centerRecipients.length === 0 ? (
                    <div className="text-center text-slate-400 text-xs italic font-bold p-6 bg-white rounded-lg border border-slate-200">
                      {isBengali
                        ? "কোনো সেন্টার সিডিউল করা নেই।"
                        : "No centers scheduled."}
                    </div>
                  ) : (
                    centerRecipients.map((cr, idx) => (
                      <div
                        key={idx}
                        className="bg-white border text-left border-slate-200 rounded-lg p-3 flex flex-col gap-2 shadow-sm pointer-events-auto"
                      >
                        <div>
                          ...
                          {cr.supervisor ? (
                            <span className="text-[10px] font-black text-emerald-600 block mt-0.5">
                              {isBengali ? "সুপারভাইজার:" : "Supervisor:"}{" "}
                              {cr.supervisor.name}{" "}
                              {cr.supervisor.phone
                                ? `(${cr.supervisor.phone})`
                                : ""}
                            ...
                            <span className="text-[10px] font-black text-rose-500 block mt-0.5">
                              {isBengali
                                ? "ডিরেক্টরিতে নাম্বার পাওয়া যায়নি"
      ...
      {isModalOpen && updatingPlan && (
        <div
          id="dtc-progress-overlay-container"
          className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm transition-all duration-200 animate-fadeIn text-left text-slate-900 font-sans"
        >
          <div className="bg-white border-2 border-slate-900 rounded-2xl max-w-xl w-full flex flex-col shadow-2xl relative overflow-hidden text-sm border-t-8 border-t-[#2563EB] animate-scale-up">
            ...
            <div className="bg-slate-50 border-b border-slate-200 p-4 space-y-1">
              <span className="text-[9.5px] uppercase font-mono tracking-wider font-extrabold text-[#2563EB] block">
                TARGET HEALTH CLINIC ENTRANCE:
              </span>
              ...
              <div className="grid grid-cols-2 gap-2 text-[10.5px] text-slate-500 pt-1 font-bold">
                <div>
                  {isBengali ? "📌 কোথায় কাজ:" : "📌 Work Areas:"}{" "}
                  <span className="text-slate-800 font-black">
                    {updatingPlan.workAreas || "-"}
                  </span>
                </div>
                <div>
                  {isBengali ? "⚠️ সমস্যা:" : "⚠️ Problem:"}{" "}
                  <span className="text-slate-800 font-black">
                    {updatingPlan.reportedIssue || "-"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="p-5 md:p-6 space-y-4 overflow-y-auto max-h-[50vh] text-xs">
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  placeholder={
                    isBengali
                      ? "যেমন: এলাকা স্প্রে করা হয়েছে..."
                      : "E.g. Spraying completed in rooms..."
                  }
                />
              </div>
              ...
              <div className="space-y-2 border-t border-slate-100 pt-3">
                <span className="text-[10px] uppercase font-mono tracking-wider font-extrabold text-[#2563EB] block flex items-center gap-1 font-black">
                  ⚙️{" "}
                  {isBengali
                    ? "কুইক টেম্পলেট সিলেক্ট"
                    : "QUICK CHOOSE TEMPLATE"}
                </span>
      ...
      {activeFormPrintItem && (
        <div
          id="print-form-modal-overlay"
          className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-sm animate-fadeIn"
        >
          <div className="bg-white border-2 border-slate-950 rounded-2xl max-w-2xl w-full flex flex-col shadow-2xl relative overflow-hidden max-h-[90vh]">
            ...
            <div className="bg-slate-900 border-b border-slate-800 p-3 flex justify-between items-center text-white shrink-0">
              <div className="flex items-center gap-2 text-xs font-mono font-black text-amber-400">
                <span>
                  📄{" "}
                  {language === "bn"
                    ? "অফিসিয়াল মেডিকেল স্যানিটেশন রিপোর্ট"
                    : "REGULATION SERVICE CLINICAL STATEMENT"}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <button
            ...
            <div
              id="clinical-printout-sheet"
              className="p-8 space-y-6 overflow-y-auto font-sans text-xs text-slate-950 bg-white leading-relaxed select-text text-left"
            >
              {/* Letterhead */}
              <div className="border-b-4 border-slate-900 pb-4 flex justify-between items-start">
                <div className="space-y-1">
                  <h2 className="text-xl font-extrabold tracking-tight text-slate-950">
                    AL WAFA STAR PEST CONTROL SERVICES
                  </h2>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wide">
                    APPROVED SANITIZATION CONTRACTOR BY MOH & DUBAI
                    MUNICIPIALITIES
                  </p>
                  <p className="text-[9.5px] text-slate-400 font-mono">
                    TEL: +971 6 565 XXXXX | EMAIL: safe.control@alwafastar.ae
                  </p>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-[9px] text-slate-450 font-mono mt-1 font-bold">
                    ERP-DOCREF: {activeFormPrintItem.id}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-300 p-3 roundedGrid p-4 rounded-xl grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <span className="text-[9px] uppercase font-mono font-bold text-slate-400 block">
                    CLINICAL SUITE FACILITY:
                  </span>
                  <p className="text-[12.5px] font-black text-slate-950">
                    {activeFormPrintItem.facilityName}
                  </p>
                  <p className="text-[10.5px] font-bold text-slate-600">
                    Assigned Official:{" "}
                    <span className="text-slate-900 underline font-black">
                      {activeFormPrintItem.assignedTo ||
                        activeFormPrintItem.postedBy}
                    </span>
                  </p>
                </div>
                <div className="space-y-1 border-l-2 border-slate-200 pl-4 font-mono text-[10px] font-bold">
                  <div>
                    <b>REFERENCE TICKET:</b> #{activeFormPrintItem.id}
                  </div>
                  <div>
                    <b>VISIT SHIFT:</b>{" "}
                    {activeFormPrintItem.dateFrame || "Archived Visit Log"}
                  </div>
                  <div>
                    <b>LOGSTAMP:</b>{" "}
                    {activeFormPrintItem.timestamp ||
                      "Active planned route window"}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="border-2 border-slate-900 p-3 rounded-lg space-y-1.5">
                  <b className="text-[10.5px] font-black text-slate-950 uppercase tracking-wide block border-b pb-1 font-mono">
                    📍{" "}
                    {isBengali
                      ? "কোথায় কোথায় কাজ করতে হবে / হয়েছে:"
                      : "WORK AREAS:"}
                  </b>
                  <p className="font-extrabold text-[#111827] text-xs">
                    {activeFormPrintItem.workAreas ||
                      "সম্পূর্ণ হসপিটাল এরিয়া ও নির্ধারিত পাইপ স্লিভস।"}
                  </p>
                </div>
                ...
                <div className="border-2 border-rose-900/50 p-3 rounded-lg bg-rose-50/5 space-y-1.5">
                  <b className="text-[10.5px] font-black text-rose-900 uppercase tracking-wide block border-b pb-1 font-mono">
                    ⚠️{" "}
                    {isBengali
                      ? "কী সমস্যা নথিভুক্ত হয়েছে:"
                      : "REPORTED ISSUE:"}
                  </b>
                  <p className="font-extrabold text-[#111827] text-xs">
                    {activeFormPrintItem.reportedIssue ||
                      "অতিরিক্ত ময়লা এবং বেকিং কর্নারে পোকামাকড়ের উৎপাত।"}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2 bg-slate-50 p-3 rounded">
                  {activeFormPrintItem.tasks ? (
                    <div>
                      <p className="text-[9.5px] text-slate-450 uppercase font-mono font-bold">
                        PRESCRIPTION CHECKLIST FOR STAFF:
                      </p>
                      <div className="flex flex-wrap gap-2 mt-1.5">
                        {activeFormPrintItem.tasks.map(
                          (tk: string, i: number) => (
                            <span
                              key={i}
                              className="bg-white border text-xs px-2 py-0.5 rounded-md font-bold font-mono"
                            >
                              ☑ {tk}
                            </span>
                          ),
                    ...
                    <div className="space-y-2">
                      <div>
                        <span className="text-[9px] uppercase font-mono font-black text-emerald-800">
                          ✓ COMPLETED SANITIZATION WORK DETAIL:
                        </span>
                        <p className="text-xs font-extrabold text-slate-900 mt-0.5 bg-white p-2 border rounded whitespace-pre-line leading-relaxed">
                          {activeFormPrintItem.doneText ||
                            "কাজের নির্ধারিত এলাকায় পেস্ট কন্ট্রোল জেল প্রয়োগ ও স্প্রে সফলভাবে সম্পন্ন করা হয়েছে।"}
                        </p>
                      </div>

                      <div>
                        <span className="text-[9px] uppercase font-mono font-black text-rose-600">
                          ⚠ PENDING / CRITICAL REMAINING NOTES:
                        </span>
                        <p className="text-xs font-bold text-slate-700 mt-0.5 bg-white p-2 border rounded whitespace-pre-line leading-relaxed italic">
                          {activeFormPrintItem.pendingText ||
                            "কোনো বকেয়া কাজ বা উপদ্রব অবশিষ্ট নেই।"}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  <tbody>
                    <tr>
                      <td className="border p-1 font-bold">
                        Target Cockroach Bait Gel V4
                      </td>
                      <td className="border p-1">MOH-APP-92-DUBAI</td>
                      <td className="border p-1">45 ml / spot</td>
                      <td className="border p-1 text-emerald-700 font-bold">
                        APPROVED & APPLIED
                      </td>
                    </tr>
                    <tr>
                      <td className="border p-1 font-bold">
                        EcoSan Biological Inhaler
                      </td>
      ...
      {isSchedulerOpen && (
        <div
          id="schedule-new-route-duty-modal"
          className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs select-none animate-fadeIn font-sans"
        >
          <div className="bg-white max-w-lg w-full rounded-2xl overflow-hidden shadow-2xl border-t-8 border-indigo-600 animate-scaleIn text-slate-800 flex flex-col max-h-[90vh]">
            ...
            <div className="p-5 border-b border-slate-100 bg-gradient-to-br from-indigo-5 to-indigo-50/50 flex justify-between items-start flex-none">
              <div className="space-y-1">
                <span className="text-[10px] bg-indigo-150 text-indigo-700 font-mono font-black tracking-widest uppercase px-2 py-0.5 rounded border border-indigo-200">
                  {isBengali
                    ? "সুপারভাইজার রুট ডিসপ্যাচ"
                    : "SUPERVISOR ROOT DISPATCH"}
                </span>
                <h3 className="text-sm font-black tracking-tight text-indigo-900 leading-tight">
                  {isBengali
                    ? "নতুন কাজের নির্দেশক ফর্ম"
                    : "Schedule New Route Duty"}
                </h3>
            ...
            <form
              onSubmit={handleCreateNewFieldPlan}
            ...
            >
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-end gap-2">
                  <label className="text-slate-950 block text-[11px] font-black uppercase">
                    {isBengali
                      ? "১. হাসপাতাল ও সেন্টার নির্বাচন:"
                      : "1. Target Hospital Facility:"}
                  </label>
                  <button
                    type="button"
                    onClick={() =>
                      setShowCustomClinicInput(!showCustomClinicInput)
                    }
                    className="text-[9px] font-black text-emerald-600 hover:text-emerald-700 bg-emerald-100/50 px-2 py-0.5 rounded shrink-0 mb-0.5 cursor-pointer transition-colors"
                  >
                    ＋ ADD NEW
                  </button>
                </div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          try {
                            ...
                            if (
                              !parsed[emLower].includes(newClinicName.trim())
                            ) {
                      ...
                      <div className="p-1 space-y-1">
                        {(() => {
                          ...
                          const addFacility = (state: string, name: string) => {
                            ...
                            if (!grouped[normalizedState])
                              grouped[normalizedState] = [];
                          ...
                          Object.entries(EMIRATE_MAPPING_FACILITIES).forEach(
                            ([em, facilities]) => {
                          ...
                          if (locations && locations.length > 0) {
                            locations.forEach((loc) =>
                              addFacility(loc.emirate || "Other", loc.name),
                            );
                          }

                          // Add from custom local storage
                          try {
                            const customCache = localStorage.getItem(
                              "ALW_CUSTOM_EMIRATE_FACILITIES",
                            );
                            if (customCache) {
                          ...
                          if (quickLinkEmirateFilter !== "All") {
                            groups = groups.filter(
                              (g) =>
                                g.toLowerCase() ===
                                quickLinkEmirateFilter.toLowerCase(),
                            );
                          }

                          return groups.map((state) => (
                            <div key={state} className="mb-2">
                              <div className="px-2 py-1 text-[11px] font-black text-slate-500 uppercase tracking-tight bg-slate-50 rounded mb-0.5">
                                {state}
                              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-slate-950 block text-[11px] font-black uppercase">
                  {isBengali
                    ? "৩. কাজের স্থান / এরিয়া নির্ধারণী (Inside or Outside):"
                    : "3. Location Area / Area Type:"}
                </label>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    const mappedVal =
                      loc.id === "Both" ? "Both Inside & Outside" : loc.id;
                    const currentSections = locationArea
                      ? locationArea.split(", ").filter(Boolean)
                      : [];
              ...
              <div className="space-y-1 pt-1">
                <label className="text-slate-950 block text-[11px] font-extrabold uppercase flex justify-between items-center">
                  <span>
                    {isBengali
                      ? "৪. কাজের বিবরণ ও নির্দেশনা (Tasks Guideline):"
                      : "4. Tasks Instruction Details:"}
                  </span>
                ...
                <textarea
                  ...
                  placeholder={
                    isBengali
                      ? "এখানে আপনার কাজের নির্দেশনা লিখুন..."
                      : "Type your tasks instructions here..."
                  }
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-3 flex-none bg-white">
                ...
                >
                  <span>
                    🚀{" "}
                    {isBengali
                      ? "মেইন প্ল্যানে সেভ করুন"
                      : "Save Plan Into Main Workbook"}
                  </span>
                </button>
              </div>
            </form>
